...
function parse(XMLBody) {
    ...
    let str ='';
    for(var i=0;i<schools.length;i++){//school length
        // console.log("in school");
        for(var j =0 ;j<schools[i].departments.length;j++){ //department length
            // console.log("in department");
            for(var k=0;k<schools[i]._departments[j].courses.length;k++){//course length
                // console.log("in course");
                var course=JSON.stringify(schools[i]._departments[j]._courses[k].toJSON());
                    // console.log("data:"+data);
                str=str+course;
                // for(var l=0;l<schools[i]._departments[j]._courses[k].sections.length;l++){//section length
                //     // console.log("添加section");
                //     var data=JSON.stringify(schools[i]._departments[j]._courses[k]._sections[l].toJSON());
                //     // console.log("data:"+data);
                //     str=str+data;
                // }
            }
